...
class SwinTransformerBlock(nn.Module):
    ...
    def __init__(
        self,
        dim,  # dim: 输入通道数
        num_heads,  # num_heads: 多头注意力的个数
        window_size=7,  # window_size: 窗口大小
        shift_size=0,
        mlp_ratio=4.0,
        qkv_bias=True,
        qk_scale=None,
        ...
        # 实现局部窗口注意力机制的核心组件，它采用了窗口内的自注意力计算。
    ...
    def forward(self, x, mask_matrix):
        ...
        # 在每个窗口内计算自注意力。注意力掩码 attn_mask 仅在 shift_size > 0 时存在。
...
class SwinTransformer(nn.Module):
    ...
    def __init__(
        ...
        # 用于视觉变换器（Vision Transformer, ViT）等模型中，将输入图像分割成若干小块（patch），
        # 然后通过卷积操作将每个块映射到一个高维的特征空间。
        # 它的作用是将图像的每个补丁表示为一个固定大小的向量，这些向量可以作为变换器（Transformer）模型的输入
