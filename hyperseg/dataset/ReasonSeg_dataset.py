...
class ReasoningSegmentation_Dataset(BaseDataset):
    """
    ...
    def __getitem__(self, index):
        ...
        json_file = self.ReasoningSegmentation_jsonfiles[index]

        # meta-file name
        meta_file_name = json_file.split("train/")[1].split(".json")[0]
        ...
        with open(json_file, "r") as f:
            ...
            object_name = data["shapes"][0]["shape_type"]
        ...
        color_name = color[0]
        prompt_for_reasoning_seg = (
            random_element + " " + prompt_for_reasoning_seg.format(color=color_name.lower(), object=object_name.lower())
        )
        ...
        original_image_ViT = original_image_ViT.resize(
            (self.ReasoningSegmentationDataset_resolution_ViT, self.ReasoningSegmentationDataset_resolution_ViT),
        ...
        original_image_SD = original_image_SD.resize(
            (self.ReasoningSegmentationDataset_resolution_for_SD, self.ReasoningSegmentationDataset_resolution_for_SD),
            resample=Image.Resampling.BICUBIC,
        )
        ...
        original_image_SD_new = original_image_SD_new.resize(
            (self.ReasoningSegmentationDataset_resolution_for_SD, self.ReasoningSegmentationDataset_resolution_for_SD),
            Image.Resampling.BICUBIC,
        )
        edited_img_SD = edited_img_SD.resize(
            (self.ReasoningSegmentationDataset_resolution_for_SD, self.ReasoningSegmentationDataset_resolution_for_SD),
            Image.Resampling.BICUBIC,
        )
        ...
        return {
            "original_img": original_image_ViT,
